...
function sendOTPEmail($email, $otp, $name)
{
    ...
    try {
        ...
        $message_body = "
            ...
            <body style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;'>
                <div style='background-color: #f8f9fa; padding: 20px; border-radius: 10px;'>
                    ...
                    <p style='text-align: center;'>Thank you for registering with SingaporePolls!</p>
                ...
                <div style='text-align: center; margin-top: 20px; padding-top: 20px; border-top: 1px solid #eee;'>
                    <p style='color: #999; font-size: 11px;'>
                        © " . date('Y') . " SingaporePolls. All rights reserved.
                    </p>
                </div>
            </body>
            </html>
        ...
        $mail->AltBody = "Hello $name,\n\nYour OTP for SingaporePolls Registration is: $otp\n\nThis OTP is valid for 1 minute.\n\nIf you did not request this, please ignore this email.";
...
function saveFormDataWithOTP($pdo, $data, $otp)
{
    try {
        $sql = "INSERT INTO sg_polls (
                  full_name, email, date_of_birth, gender, address, postcode, otp, otp_verified, status, otp_created_at, created_at
                ) VALUES (
                  :full_name, :email, :date_of_birth, :gender, :address, :postcode,
                  :otp, 0, 'pending', NOW(), NOW()
                )";
...
function verifyOTPFromDB($pdo, $email, $otp)
{
    try {
        ...
        if (!$user) {
            ...
            $checkSql = "SELECT id, otp, otp_created_at FROM sg_polls 
...
{
    try {
        $sql = "SELECT id FROM sg_polls WHERE email = :email AND status = 'verified' LIMIT 1";
        $stmt = $pdo->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_otp') {
    ...
    if ($pdo === null) {
        ...
        if (
            ...
            empty($gender) || empty($address) || empty($postcode)
        ) {
            ...
            if ($user_id) {
                ...
                if ($email_sent) {
                    ...
                    $deleteSql = "DELETE FROM sg_polls WHERE id = :id";
