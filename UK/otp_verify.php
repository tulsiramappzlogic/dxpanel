...
function verifyOTPFromDB($pdo, $email, $otp)
{
    try {
        ...
        $sql = "SELECT id, otp, otp_created_at FROM uk_polls 
        ...
        if (!$user) {
            ...
            $checkSql = "SELECT id, otp, otp_created_at FROM uk_polls 
        ...
        $updateSql = "UPDATE uk_polls SET otp_verified = 1, status = 'verified', updated_at = NOW() WHERE id = :id";
...
{
    try {
        $sql = "SELECT id FROM uk_polls WHERE email = :email AND status = 'verified' LIMIT 1";
        $stmt = $pdo->prepare($sql);
        ...
        if (
          ...
          empty($country) || empty($postcode)
            ...
            if ($user_id) {
                ...
                $email_sent = \sendOTPEmail('UK', $email, $full_name, $otp);
                ...
                if ($email_sent) {
                    ...
                    $deleteSql = "DELETE FROM uk_polls WHERE id = :id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'resend_otp') {
    ...
    if ($otp_updated) {
        // Send new OTP email
                $email_sent = \sendOTPEmail('UK', $email, $user_name, $otp);
