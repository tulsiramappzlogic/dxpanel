...
{
    try {
        $sql = "INSERT INTO ph_polls (
                  full_name, email, date_of_birth, gender, address, barangay, city, 
...
function verifyOTPFromDB($pdo, $email, $otp)
{
    try {
        ...
        $sql = "SELECT id, otp, otp_created_at FROM ph_polls 
        ...
        if (!$user) {
            ...
            $checkSql = "SELECT id, otp, otp_created_at FROM ph_polls 
        ...
        $updateSql = "UPDATE ph_polls SET otp_verified = 1, status = 'verified', updated_at = NOW() WHERE id = :id";
...
{
    try {
        $sql = "UPDATE ph_polls SET otp = :otp, otp_created_at = NOW(), otp_verified = 0, status = 'pending' 
...
{
    try {
        $sql = "SELECT id FROM ph_polls WHERE email = :email AND status = 'verified' LIMIT 1";
        $stmt = $pdo->prepare($sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'send_otp') {
    ...
    if ($pdo === null) {
        ...
        $barangay = sanitizeString($_POST['barangay'] ?? '');
        ...
        if (
            ...
            empty($country) || empty($postcode) || empty($barangay)
        ) {
            ...
            if ($user_id) {
                ...
                if ($email_sent) {
                    ...
                    $deleteSql = "DELETE FROM ph_polls WHERE id = :id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'resend_otp') {
    ...
    try {
        $nameSql = "SELECT full_name FROM ph_polls WHERE email = :email AND status = 'pending' LIMIT 1";
